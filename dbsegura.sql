...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-04-2025 a las 04:56:30
...
(1, '2025-03-26 03:18:42', 1, 1, 15000.00, NULL),
(2, '2025-03-26 03:18:42', 2, 2, 12000.00, NULL),
(3, '2025-03-26 03:28:47', 3, 3, 18000.00, NULL),
(4, '2025-03-26 03:28:47', 4, 4, 25000.00, NULL),
(5, '2025-03-26 03:28:47', 5, 5, 14000.00, NULL),
(6, '2025-03-26 03:33:21', 1, 1, 25000.00, NULL);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_auditoria_resumen`  AS SELECT `a`.`id` AS `id`, `u`.`nombre` AS `usuario`, `a`.`ip` AS `ip`, `a`.`accion` AS `accion`, `a`.`fecha` AS `fecha` FROM (`auditoria` `a` join `usuario` `u` on(`a`.`usuario_id` = `u`.`id_usuario`)) ;
